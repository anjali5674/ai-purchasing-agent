...
def _seed_supplier_products(db: Session):
    ...
    mappings = [
        ...
        # Product 2 (Cold Brew) — supplied by BevCo
        SupplierProduct(supplier_id=2, product_id=2, unit_price=3.50),
        # Product 3 (Protein Bars) — supplied by NutriSource
...
def _seed_inventory(db: Session):
    ...
    inventories = [
        ...
        Inventory(product_id=3, node_id=2, current_quantity=100),
        # Node 3 — South Micro (small node, limited stock)
        Inventory(product_id=1, node_id=3, current_quantity=40),
        Inventory(product_id=3, node_id=3, current_quantity=50),
        Inventory(product_id=4, node_id=3, current_quantity=25),
        Inventory(product_id=5, node_id=3, current_quantity=60),
    ]
...
def _seed_forecasts(db: Session):
    forecasts = [
        # Node 1 forecasts
        DemandForecast(product_id=1, node_id=1, forecast_quantity=500, forecast_period="2026-Q4"),
        DemandForecast(product_id=2, node_id=1, forecast_quantity=280, forecast_period="2026-Q4"),
        DemandForecast(product_id=3, node_id=1, forecast_quantity=800, forecast_period="2026-Q4"),
        DemandForecast(product_id=4, node_id=1, forecast_quantity=350, forecast_period="2026-Q4"),
        DemandForecast(product_id=5, node_id=1, forecast_quantity=250, forecast_period="2026-Q4"),
        # Node 2 forecasts
        DemandForecast(product_id=1, node_id=2, forecast_quantity=1000, forecast_period="2026-Q4"),
        DemandForecast(product_id=2, node_id=2, forecast_quantity=600, forecast_period="2026-Q4"),
        # Node 3 forecasts
        DemandForecast(product_id=3, node_id=3, forecast_quantity=100, forecast_period="2026-Q4"),
        DemandForecast(product_id=4, node_id=3, forecast_quantity=100, forecast_period="2026-Q4"),
        DemandForecast(product_id=5, node_id=3, forecast_quantity=180, forecast_period="2026-Q4"),
    ]
...
def _seed_purchase_orders(db: Session):
    """Existing open POs that the agent should consider."""
    orders = [
        # Open PO for Almonds at Node 1 — 200 units already ordered
        PurchaseOrder(
            product_id=1, supplier_id=1, node_id=1,
            quantity=200, unit_price=11.00, total_price=2200.00,
            status=PurchaseOrderStatus.OPEN,
            created_at=datetime(2026, 8, 15, tzinfo=timezone.utc),
        ),
        # Completed PO for Cold Brew
        PurchaseOrder(
            product_id=2, supplier_id=2, node_id=1,
            quantity=150, unit_price=3.50, total_price=525.00,
            status=PurchaseOrderStatus.RECEIVED,
            created_at=datetime(2026, 8, 10, tzinfo=timezone.utc),
        ),
        # Open PO for OJ at Node 3
        PurchaseOrder(
            product_id=4, supplier_id=3, node_id=3,
